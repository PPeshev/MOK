    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        if (fFirstRun){
            fModifierV2 = pindex->nHeight >= Params().Zerocoin_StartHeight();
            fFirstRun = false;
        }
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZMokStake(spend));
    } else {
        ...
        CMokStake* mokInput = new CMokStake();
        mokInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(mokInput);
    }
