...
CzMOKTracker::CzMOKTracker(std::string strWalletFile)
{
...
CzMOKTracker::~CzMOKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMOKTracker::Init()
{
...
bool CzMOKTracker::Archive(CMintMeta& meta)
{
...
bool CzMOKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMOKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMOKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMOKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMOKTracker::GetSerialHashes()
{
...
CAmount CzMOKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMOKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMOKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMOKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMOKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMOKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMOKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMOKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMOKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMOKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMOKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMOKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMOKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMOKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMOKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMOKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMOKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmok from DB\n", __func__, listDeterministicDB.size());
    }
