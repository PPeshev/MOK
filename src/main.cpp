...
#if defined(NDEBUG)
#error "MOKEN cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 1;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in umok) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zMOK is properly signed
    if (pindex->nHeight >= Params().Zerocoin_StartHeight()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zMOK spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zMOK without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zMOK spend with serial %s is already in block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMOK spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmok");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zMOK mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 90 * COIN;
    } else if (nHeight < 518400 && nHeight >= 172800) { 
        nSubsidy = 60 * COIN;
    } else if (nHeight < 1036800 && nHeight >= 518400) { 
        nSubsidy = 50 * COIN;
    } else if (nHeight < 1555200 && nHeight >= 1036800) { 
        nSubsidy = 40 * COIN;
    } else if (nHeight < 2073600 && nHeight >= 1555200) { 
        nSubsidy = 30 * COIN;
    } else if (nHeight < 2592000 && nHeight >= 2073600) {
        nSubsidy = 12.5 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZMOKStake)
{
    int64_t ret = 0;
...
{
    bool isInvalid = nHeight >= Params().Zerocoin_StartHeight() && invalid_out::ContainsOutPoint(out);
    return !isInvalid;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MOKEN
...
{
    RenameThread("moken-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMOKMinted()
{
...
void RecalculateZMOKSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMOK supply
...
bool RecalculateMOKSupply(int nHeightStart)
{
...
bool UpdateZMOKSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateMOKSupply(Params().Zerocoin_StartHeight());
    }

    //Track zMOK money supply in the block index
    if (!UpdateZMOKSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zMOK supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMokSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MOK to zMOK
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMOK spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMOK serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_StartHeight())
        return error("%s: zMOK stake block is less than allowed start height", __func__);

    if (CZMokStake* zMOK = dynamic_cast<CZMokStake*>(stake)) {
        CBlockIndex* pindexFrom = zMOK->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zMOK stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zMOK stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zMOK->GetValue()));
        if (nChecksum200 != zMOK->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zMOK->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZMOK() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zMOK fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMOK mints and %d zMOK spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MOKEN: We use certain sporks during IBD, so check to see if they are
