...
public:
    ...
    bool WriteZMOKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMOKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMOKSeed_deprecated(uint256& seed);
    bool EraseZMOKSeed();
    bool EraseZMOKSeed_deprecated();

    bool WriteZMOKCount(const uint32_t& nCount);
    bool ReadZMOKCount(uint32_t& nCount);
