...
#include "ui_zmokcontroldialog.h"
...
std::set<std::string> ZMokControlDialog::setSelectedMints;
std::set<CMintMeta> ZMokControlDialog::setMints;

ZMokControlDialog::ZMokControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMokControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMokControlDialog::~ZMokControlDialog()
{
    delete ui;
}

void ZMokControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMokControlDialog::updateList()
{
...
void ZMokControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMokControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMok_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMokControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMokControlDialog::GetSelectedMints()
{
...
void ZMokControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
