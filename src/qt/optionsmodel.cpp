...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MOK);
    ...
    if (!settings.contains("nAnonymizeMokenAmount"))
        settings.setValue("nAnonymizeMokenAmount", 1000);

    nAnonymizeMokenAmount = settings.value("nAnonymizeMokenAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMokenAmount"))
        SoftSetArg("-anonymizemokenamount", settings.value("nAnonymizeMokenAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in moken.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMokenAmount:
            return QVariant(nAnonymizeMokenAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMokenAmount:
            nAnonymizeMokenAmount = value.toInt();
            settings.setValue("nAnonymizeMokenAmount", nAnonymizeMokenAmount);
            emit anonymizeMokenAmountChanged(nAnonymizeMokenAmount);
            break;
        case CoinControlFeatures:
