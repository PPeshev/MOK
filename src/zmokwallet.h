...
#ifndef MOKEN_ZMOKWALLET_H
#define MOKEN_ZMOKWALLET_H
...
public:
    CzMOKWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMOK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMOK(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MOKEN_ZMOKWALLET_H
