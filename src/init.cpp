...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "moken.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mokend.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MOK and zMOK money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 10137, 11137));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MOK/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MOK/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, moken, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MOK/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MOKEN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mokstake=<n>", strprintf(_("Enable or disable staking functionality for MOK inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmokstake=<n>", strprintf(_("Enable or disable staking functionality for zMOK inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:10137"));
    ...
    strUsage += HelpMessageOpt("-backupzmok=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMOK minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmokbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMOK backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizemokenamount=<n>     " + strprintf(_("Keep N MOK anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The Moken Core Developers")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that MOKEN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MOKEN Core is shutting down."));
...
#endif
    // Make sure only a single MOKEN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MOKEN Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //MOKEN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MOKEN specific: zerocoin and spork DB's
                ...
                // MOKEN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMOKMinted();
                        RecalculateZMOKSpent();
                    }
                    RecalculateMOKSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        ...
                        // MOKEN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MOKEN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MOKEN Core to complete") << "\n";
        ...
        zwalletMain = new CzMOKWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zMOKWallet
        uiInterface.InitMessage(_("Syncing zMOK wallet..."));

        bool fEnableZMokBackups = GetBoolArg("-backupzmok", true);
        pwalletMain->setZMokAutoBackups(fEnableZMokBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zmokTracker->Init();
...
//    }
//
//    nAnonymizeMokenAmount = GetArg("-anonymizemokenamount", 0);
//    if (nAnonymizeMokenAmount > 999999) nAnonymizeMokenAmount = 999999;
//    if (nAnonymizeMokenAmount < 2) nAnonymizeMokenAmount = 2;
    ...
    LogPrintf("Anonymize MOKEN Amount %d\n", nAnonymizeMokenAmount);
    ...
    /* Denominations
       ...
       For example:
       1MOK+1000 == (.1MOK+100)*10
       10MOK+10000 == (1MOK+1000)*10
    */
