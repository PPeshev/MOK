...
CzMOKWallet::CzMOKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmok seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMOKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMOKSeed_deprecated()) {
                    LogPrintf("%s: Updated zMOK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmok seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmok wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMOKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMOKCount(nCountLastUsed);
    else if (!walletdb.ReadZMOKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMOKWallet::Lock()
{
    seedMaster = 0;
}

void CzMOKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMOKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMOK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMOKWallet::LoadMintPoolFromDB()
{
...
void CzMOKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMOKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMOKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmokTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMOKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMOK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmokTracker which also adds to database
    pwalletMain->zmokTracker->Add(dMint, true);
...
void CzMOKWallet::SeedToZMOK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMOKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMOKWallet::GenerateDeterministicZMOK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMOKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMOKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
