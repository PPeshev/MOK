...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
	...
	if (!fProofOfStake) nBlockMaxSizeNetwork = MAX_BLOCK_SIZE_LEGACY;
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zmoks that have been in mempool long
                    //and higher priority to zmoks that are large in value
                    ...
                    // zMOK spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
			...
			if (!fProofOfStake) nMaxBlockSigOps = MAX_BLOCK_SIGOPS_LEGACY;
            ...
            // double check that there are no double spent zMOK spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        ...
                        bool fUseV1Params = false; //libzerocoin::ExtractVersionFromSerial(spend.getCoinSerialNumber()) < libzerocoin::PrivateCoin::PUBKEY_VERSION;
                ...
                //This zMOK serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (!fProofOfStake) {
			...
			else{//remove else in POS mode
				pblock->payee = txNew.vout[0].scriptPubKey;
			}
        }
        ...
        if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
            //For the period before v2 activation, zMOK will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MOKENMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zmokTracker->RemovePending(pblock->vtx[1].GetHash());
        return error("MOKENMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("MOKENMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("moken-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zMOK with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zMOK key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zMOK key failed \n");
                    continue;
                }
        ...
        LogPrintf("Running MOKENMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
